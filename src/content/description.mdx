...
1.  Bacterial receptors are incredibly diverse. Each RBP is highly specific to each given receptor, generative models would need to encompass a vast range of sequence and structural diversity to generalize across necessary receptor classes making de novo generation very difficult.
...
To overcome these challenges, we built **PHORAGER** – a generative phage bank to develop RBPs for a diverse set of targets. First, PHORAGER takes in a bacterial genome as input and identifies its receptor types. From this, it finds bacteria that are most likely to have these receptors and the corresponding phages that infect them. If these phages show high affinity, they are recommended; otherwise, they are used as templates for generation.

Here, the receptor binding domains (RBDs) from the highest affinity phages are input into ESM3 – a masked protein language model, to hallucinate new residues that confer greater affinity to the target receptor. These variants are tested by co-folding the outputs and the target using Boltz-2 – a diffusion-based protein folding model, and observing the resulting structure and metrics. By repeating this process over many trials using Markov Chain Monte Carlo (MCMC), PHORAGER converges on better and better binders with each iteration. For *in silico* validation, molecular docking with Haddock 3 is used. Promising candidates are then codon-optimized and sent to the wet lab team for in vitro validation.
...
Afterwards, the prophage is induced into the lytic cycle via the antibiotic mitomycin C, allowing the mutant phages to be collected and purified. To test their ability to infect bacteria, the mutants are spotted on lawns of K12 with diverse receptor types, and results are returned to the dry lab team. The results from this spotting experiment will then be sent back to the dry lab team to further inform the PHORAGER pipeline.

 
## EVADE: Electroceutical Vehicle for Antimicrobial Delivery

While AI-powered phage therapies hold great promise for treating infections, their applications extend beyond traditional sites. In the GI tracts of both humans and livestock exists the gut resistome – the collection of antibiotic resistance genes (ARGs) directly associated with the development and spread of ABR infections @SciDirect2025S2772416625000464. The resistome is dynamic in time and space, and can become enriched for harmful ARGs when placed under selective pressure from antibiotics @SciDirect2025S2772416625000464. This is particularly concerning in agriculture, which accounts for over half of all antibiotic use (in much less regulated fashions compared to clinical settings) @terKuile2016LowAntibioticsAgriculture.
...
This year, we sought to develop Mystiphage into a go-to-market stage company, integrating all aspects of our project into investor-ready deliverables. Through an incubator program, we refined our problem statement, value proposition, and business model. We developed a full business plan, 6-minute pitch, and other items necessary to engage in pre-seed and seed funding rounds.

By commercializing Mystiphage, we aim to navigate the complex regulatory landscape of phage therapy and generative protein therapies to translate our work to a global community in need.
...
Antibiotic resistance is one of humanity's largest global health issues,
being projected to cause ~39 million cumulative deaths between now and 2050 @Lancet. Phage therapy is a promising solution and has had success in multiple
high-profile cases @CNNHealth @CBCNews. However, the process of finding a phage with the correct binding receptor-binding protein (RBP) for its target bacterial receptor (a process called phage matching) is
highly time consuming @Frontiers. Studies have shown that slow phage matching causes months-long
wait times @OxfordAcademic and
single-digit fulfillment rates @Nature for phage therapies – this prevents this treatment from effectively scaling and meeting the demands of millions of future cases where antibiotics might fail.
...
Phages target either glycan or protein structures specific to a certain bacterial strain decreasing the risk of cross reactivity. Phages infect bacterial cells by first binding to a bacterial surface using structures called tail fibers. Some tail fibers have enzymatic activity that can degrade bacterial capsules to facilitate easier binding. Following surface binding, phages then inject their genome into the bacterial cell which eventually leads to the production of new phage particles which are assembled within the cell. The bacterial cell then gets lysed and the new phages
spread to new bacterial cells @ofir2018contemporary. Phage therapy tries to harness this biological process to fight bacterial infections, but currently its clinical use has multiple limitations. The use of phage therapy has been mainly limited by phage screening - the process of identifying specific bacteriophages (viruses that infect bacteria) that can effectively target and destroy a patient's bacterial infection.

  

  

The phage screening process often involves searching through thousands of phages for a match to a specific bacterial target, making it a long and extensive process
@ScienceDirect. One of the methods used currently to address these problems is the use of phage cocktails, which are mixtures of several different phages given at once in the hope that at least one will infect the target bacteria. The phage cocktail approach, however, risks increasing resistance and reducing phage selectivity, undermining their justification as a good solution
to begin with @ScientificReport. Studies have shown that the most critical metric for determining whether a phage can bind to a bacteria is the RBP (receptor binding protein)
of the phage @ScienceDirect2 @SpringerNature3. The development and use of recombinant phages that are engineered to bind to a custom bacterial target by genetically altering its RBP is a relatively novel concept and lacks sufficient data surrounding phage RBP-host interactions. This is another significant obstacle to the practical
use of phage therapy @NatureMicrobiology. Overcoming these major obstacles to the use of phage therapy therefore requires the use of powerful AI-based computational models.
...
The protein design challenge resembles searching for a single golden key in an ocean of near-identical fakes – except the ocean contains 20^300 possible sequences for a typical 300-residue protein, a combinatorial explosion that dwarfs the number of atoms in the
observable universe @currin2015synthetic.

  

  

Recent advances in AI-driven protein design, such as ESM, AlphaFold, and RoseTTAFold, have revolutionized our ability to model and engineer functional proteins. Yet for all their power, these methods have not been applied to the targeted design of synthetic receptor-binding proteins (RBPs) to redirect phage host specificity, a crucial unmet need in the phage therapy domain where bacterial resistance patterns shift faster than traditional phage
isolation can follow @doud2025optimizing. The challenge proves particularly acute: phage RBPs across different families exhibit extreme sequence and structural variability—homology often vanishes entirely even between functionally analogous proteins binding similar bacterial
receptors @dion2020phage. It's molecular convergent evolution at its most dramatic, where nature has discovered thousands of distinct solutions to the same recognition problem, leaving AI models trained on homology-rich protein families struggling to extract generalizable design principles.

  

  

AlphaFold3 represents the current apex of structure prediction, visualizing three-dimensional conformations with near-experimental accuracy from amino acid sequences alone. This capability has transformed protein engineering from blind groping in sequence space
to informed navigation @Nature5. However, AlphaFold3's Achilles heel emerges precisely where phage RBPs operate: it lacks built-in modules for modeling protein-ligand or protein-glycan
interactions @DeepOrigin. These binding modes prove critical for phage RBPs, which must recognize and engage highly variable bacterial surface receptors—lipopolysaccharides, teichoic acids, capsular polysaccharides—whose conformational flexibility and chemical diversity fall outside AlphaFold's protein-centric training regime.

  

  

Boltz-2 pushes protein modeling further by coupling AlphaFold-like structural prediction with an affinity prediction module. It evaluates the strength of binding interactions between candidate proteins and their targets, including glycans
and membrane proteins @PubMed5. This dual capability is novel and important for phage research as it allows us to predict how well a designed RBP will bind to a bacterial receptor, therefore linking structure and function within a single model.
...
Phage therapy, synthetic biology, and AI-driven protein design are converging to create unprecedented opportunities for fighting AMR. PHORAGER harnesses these advances to pursue the following objectives:
...
## Design/Methods
  ...
  ![fig1 workflow](https://static.igem.wiki/teams/5708/drylab/fig1-workflow.webp "**Figure 1. PHORAGER generative phage bank workflow.** Overview of the computational pipeline for generating and validating novel receptor-binding proteins. The workflow integrates a user-friendly interface for clinical applications, computational sequence generation through iterative MCMC refinement, and experimental validation of designed sequences.”)
...
Accurate identification of bacterial surface receptors is critical for the Generative Phage Bank routing system. When a novel bacterial sequence is submitted, receptor annotation is the first computational step, determining whether pre-validated phages can be retrieved or if de novo RBP design must be initiated. We developed comprehensive annotation pipelines for both glycan and protein receptors.
...
Fourteen major protein receptors are known to serve as phage binding targets in E. coli: BamA, BtuB, FadL, FepA, FhuA, LamB, LptD, NfrA, OmpA, OmpC, OmpF, TolC, Tsx,
and YncD @hantke2020compilation. We curated one to two representative amino acid sequences for each receptor and used DIAMOND, an accelerated BLAST implementation, to identify homologs across all genomes.
...
Physics-based molecular docking provides essential orthogonal validation for machine learning-based binding affinity predictions in de novo protein design pipelines. The consensus across recent literature is that ML and physics-based approaches are complementary rather than competitive, with hybrid pipelines combining the speed of ML with the rigor of physics-based methods achieving the best results. ML models like AlphaFold3 and Boltz-2 excel at rapid initial predictions but exhibit critical limitations including stereochemistry violations, inability to model conformational changes, and susceptibility to generating adversarial sequences that appear valid computationally but fail experimentally. Physics-based docking tools like HADDOCK3 address these limitations through explicit physical modeling, providing confidence in predictions that pure ML approaches
cannot guarantee @krokidis2025alphafold3.

  

Recent assessments of AlphaFold3 and similar ML models reveal fundamental limitations that necessitate physics-based validation. Abramson et al. demonstrated in their Nature 2024 paper that AlphaFold3 achieves only 95.6% stereochemical validity, exhibiting a 4.4% chirality violation rate and occasional production of overlapping atoms that violate basic physical chemistry. These artifacts arise because ML models learn statistical patterns from training data rather than enforcing physical constraints. Chen et al. found that while AlphaFold3 achieves good correlation (R=0.86) for predicting binding free energy changes upon mutation, some complex structures contain large errors not captured by internal confidence metrics like ipTM, leading them to conclude that independent validation of AlphaFold3 predictions is necessary.

  

The challenge with ML models fundamentally stems from their optimization objectives. Notin et al. noted in their Nature Biotechnology 2024 review that in silico metrics such as native sequence recovery can be misleading, while Baker observed in his Cell 2024 perspective that the insensitivity of current protein structure prediction methods to amino acid point mutations that can be catastrophic in protein design necessitates orthogonal validation. ML models are optimized for structure prediction accuracy on natural sequences, not for distinguishing viable from non-viable designed sequences, creating a critical gap that physics-based validation fills.
...
HADDOCK3 (Giulini et al., 2025) represents evolutionary thinking in molecular docking—breaking the original rigid pipeline into modular components that adapt to diverse validation scenarios. This architectural flexibility positions HADDOCK3 as the ideal physics-based complement to ML predictions in the post-AlphaFold era.
...
**HADDOCK Score** = w<sub>vdw</sub>E<sub>vdw</sub> + w<sub>elec</sub>E<sub>elec</sub> + w<sub>desolv</sub>E<sub>desolv</sub> − w<sub>BSA</sub> · BSA + w<sub>AIR</sub>E<sub>AIR</sub>

  

where E<sub>vdw</sub> and E<sub>elec</sub> capture van der Waals and Coulombic interactions through molecular mechanics force fields, E<sub>desolv</sub> approximates desolvation via empirical atomic solvation parameters, BSA (buried surface area) captures hydrophobic effects, and E<sub>AIR</sub> penalizes violations of ambiguous interaction restraints. The weights shift dramatically across refinement stages: rigid-body docking uses w<sub>vdw</sub>=0.01 to allow initial penetration, semi-flexible refinement increases to w<sub>vdw</sub>=1.0 for realistic contacts, while explicit solvent refinement damps electrostatics to w<sub>elec</sub>=0.2. This empirical dampening implicitly accounts for polarization and local rearrangement that simplified models miss, encoding decades of validation against experimental binding thermodynamics.
...
HADDOCK3 demonstrates unique advantages for protein-glycan interactions—precisely where ML models trained predominantly on protein-protein interfaces stumble. Ranaudo et al. (2024) benchmarked HADDOCK across 89 high-resolution glycan complexes, achieving 70% success for bound docking and 40% for unbound when binding sites are known. The protocol handles diverse systems (antibodies, lectins, viral glycan-binding proteins, enzymes) that span the functional space of carbohydrate recognition.
...
Systematic parameter exploration addresses a subtle but crucial weakness in docking: sensitivity to arbitrary parameter choices in rugged, non-convex energy landscapes. Bender et al.'s (2021) practical guide on large-scale docking emphasizes parameter evaluation before screening.

  

Latin hypercube sampling (LHS) provides stratified coverage of parameter space by dividing each parameter's range into N equiprobable intervals and sampling once per interval, ensuring no two samples share the same interval for any parameter. For k parameters, this yields N samples with guaranteed coverage across all dimensions—versus N<sup>k</sup> samples required for equivalent full factorial coverage. This efficiency proves essential when each docking simulation costs substantial computational resources.
...
Successful pipelines adopt a cascade strategy: rapid ML-based exploration generates diverse candidates, physics-based filtering eliminates physically implausible designs, and parameter sweeps quantify confidence. Dauparas et al. (2022) demonstrated this with ProteinMPNN, where ML sequence design rescued 73 of 96 previously failed attempts, with 50 maintaining desired oligomeric states—but the authors emphasized that structure prediction evaluates purely in silico while design methods cannot, as computational metrics prove sensitive to architecture without correlating reliably with experimental success.

  

The most sophisticated implementations employ parameter sweeps during physics-based validation to quantify robustness. Rather than accepting scores at face value, varying weights, solvation models, and sampling protocols through LHS identifies predictions robust across parameter space versus those sensitive to arbitrary choices. Wu et al.'s (2025) FDA (Folding-Docking-Affinity) framework found ColabFold-generated apo-structures surprisingly improved affinity predictions versus crystallized holo-structures—suggesting physics-based docking captures induced-fit effects ML structure prediction misses.

  

In our MCMC-driven RBP design pipeline, HADDOCK3 serves this critical validation role. After iterative ESM3 generation guided by Boltz-2's affinity oracle, final candidates undergo full physics-based docking simulations with both wild-type and target glycan receptors. The modular flexref protocol for glycan docking, combined with LHS parameter sweeps across scoring weights and solvation models, identifies designs that satisfy both ML pattern-matching and explicit physical constraints—candidates robust enough to warrant experimental synthesis.
...
**[Note: 3 carousels, 1 for each batch of sequences. Whatever results are associated with that batch we will show]**
...
![fig2](https://static.igem.wiki/teams/5708/drylab/fig2.avif "**Figure 2. Validation and structural characterization of glycan receptor-binding designs (Batch 1).** **(A)** BLASTp analysis of top 10 generated sequences against the NCBI *nr* database confirms sequence novelty. **(B)** Structure of P2_gpH_2 RBD (blue) in complex with TFA chaperone (red) and truncated R1 glycan (black). **(C)** Structure of P2_gpH_8 RBD (blue) with TFA chaperone (red) and truncated R1 glycan (black). Both sequences passed all validation criteria and were experimentally synthesized.")
...
![fig3](https://static.igem.wiki/teams/5708/drylab/fig3.avif "**Figure 3. Protein receptor-binding designs and structural models (Batch 1).** **(A)** BLASTp validation of top 8 generated sequences against the NCBI *nr* database. **(B)** Structural models showing trimeric assemblies of HK97 and Lambda RBDs (blue) bound to trimeric OmpC protein receptor (orange), with coordinated ions (green).")
...
![fig4-1](https://static.igem.wiki/teams/5708/drylab/fig4-1.avif "**Figure 4. MCMC sampling dynamics for Mu gp49 RBP optimization.** **(A)** Acceptance rate decay across three independent MCMC chains over 10,000 iterations, indicating convergence toward optimal sequence space. **(B)** Overall acceptance statistics across chains, with mean acceptance rate of 34.9%.")
...
![fig5-1](https://static.igem.wiki/teams/5708/drylab/fig5-1.avif "**Figure 6. MCMC sampling dynamics for Mu gp52 RBP optimization.** **(A)** Acceptance rate trajectories for three independent chains demonstrating convergence behavior. **(B)** Cumulative acceptance statistics showing mean acceptance rate of 40.3% across all chains.")
![fig5-2](https://static.igem.wiki/teams/5708/drylab/fig5-2.avif "**Figure 7. Energy landscape analysis for Mu gp52 target retargeting.** **(A)** Energy convergence profiles for three MCMC chains. **(B)** Wild-type K12 versus truncated R1 target binding energy comparison. Optimization successfully drives all chains toward enhanced R1 binding, with final target energies exceeding wild-type energies.")
...
![fig6-1](https://static.igem.wiki/teams/5708/drylab/fig6-1.avif "**Figure 8. MCMC sampling dynamics for P2 gpH RBP optimization.** **(A)** Acceptance rate evolution across three independent chains. **(B)** Overall acceptance rate of 33.9% across all sampling chains.")
![fig6-2](https://static.igem.wiki/teams/5708/drylab/fig6-1.avif "**Figure 9. Energy landscape analysis for P2 gpH target retargeting.** **(A)** Energy trajectories showing convergence across three independent chains. **(B)** Binding energy comparison between wild-type K12 and truncated R1 target. Consistent optimization toward enhanced R1 binding across all chains, with target energies surpassing wild-type values. ")
...
![fig8](https://static.igem.wiki/teams/5708/drylab/fig8.avif "**Figure 11. Combined clustering analysis across MCMC chains.** **(A)** Mean cluster energy versus iteration for pooled chains within each RBP. Later clusters correspond to higher-energy local maxima. **(B)** Global trend across all RBPs demonstrating that sampling progression correlates with improved binding energies.")
...
![fig9](https://static.igem.wiki/teams/5708/drylab/fig9.avif "**Figure 12. Energy distributions of top candidate sequence clusters.** Energy profiles for sequences within the top 9 clusters selected from combined clustering analysis. Each panel represents a distinct cluster, showing variation in sequence diversity and energy distributions. Higher-energy sequences are preferentially represented across clusters.")
...
![fig10](https://static.igem.wiki/teams/5708/drylab/fig10.avif "**Figure 13. Sequence novelty validation via homology search.** **(A)** BLASTp analysis of representative sequences from top 9 clusters against the NCBI *nr* database reveals minimal homology to known proteins, confirming sequence novelty. **(B)** Control BLASTp analysis of wild-type template sequences confirms expected homology to characterized proteins.")
...
![fig11](https://static.igem.wiki/teams/5708/drylab/fig11.avif "**Figure 14. Structural models of top candidate sequences.** Three-dimensional structures of representative sequences from each of the top 9 clusters, illustrating structural diversity of designed receptor-binding proteins.")
...
Finally, preliminary trials revealed that RFdiffusion-generated backbones frequently failed to produce stable folds when adapted to multi-domain or glycan-bound RBP architectures, reflecting priors optimized for soluble monomeric proteins rather than flexible, surface-tethered viral attachment systems. For these reasons, PHORAGER adopts a sampling-centric framework that combines structure-aware scoring, adaptive refinement, and experimental feedback loops—trading unconditional generativity for biologically grounded precision.
...
**Enhance the active-learning loop.** We plan to implement a Cross-Entropy Method (CEM) optimization strategy [The Cross-Entropy Method for Optimization - ScienceDirect]. CEM iteratively samples a batch of sequences, selects the top-performing "elite" candidates based on experimental or predicted scores, and updates the generative model to increase the probability of sampling high-function variants. By using multiple performance metrics—including wet-lab growth curves, binding assays, and Boltz-2 scores—we can guide the model toward near-optimal solutions. A continuously growing database of positive and negative experimental outcomes will allow us to perform multiple sequence alignments on successful RBPs to identify conserved residues for freezing, while BLAST can remove designs similar to known failures.
...
**Toward ethical and equitable deployment.** Computational phage design raises questions about access and oversight. Phage therapies, especially those designed via AI, must be accompanied by regulatory frameworks to ensure safety, prevent misuse, and make treatments available in regions most burdened by antimicrobial resistance. A global generative phage bank, shared openly with healthcare providers, could democratize access while enabling rapid response to emergent pathogens.

  

In sum, PHORAGER does not simply predict – it proposes. It charts a map across the vast unknowns of viral-host recognition, where every designed sequence is both a hypothesis and a candidate therapy. By coupling generative design with continuous experimental feedback, PHORAGER moves phage engineering from empirical discovery toward predictive control—an essential leap as we confront a post-antibiotic world.
